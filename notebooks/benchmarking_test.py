...
# MAGIC %md ### Time Test
# MAGIC Test rates calculations with a large number buildings (100K) with electric loads only. On Rates Cluster, Driver = 64 GB, 8 cores:
# MAGIC 1. Building test data : 15s
# MAGIC 2. Construct load profile numpy matrix: 19s
# MAGIC 3. Calculating bills: 1s
# MAGIC
...
# MAGIC Note that if we were modeling all fuel loads, then we would likely reach a bottleneck with (1, building test data) with roughly 4x more data, but not with steps 2-3 which are done for one fuel at a time. 
# MAGIC
...
def generate_large_load_profile(num_buildings:int=5000):
    """
